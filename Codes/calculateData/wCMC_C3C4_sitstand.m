...
function wCMC_C3C4_sitstand(eventChar, TL, subj_no, setPath)
    ...
    for j = 1:length(EMG_labels)
        ...
        save_folder = fullfile(config.dataBaseFolder, "CMCresult_lowerLimb", subj_no, filename, eventChar);
        if ~exist(save_folder, "dir")
            mkdir(save_folder)
        end
    
        save_path = fullfile(save_folder, aEEG.setname + "_CMCvalues_" + label + ".mat");
